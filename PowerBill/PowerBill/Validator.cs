...
{ 
    ...
    public static class Validator
    {
        ...
        public static bool IsPresentlbl(Label lb, string word)
        {
            bool isValid = true; // "innocent until proven guilty"
            if (lb.Text == "")// bad!
            {
                isValid = false;
                MessageBox.Show(word + " has to be provided", "Input Error");
                lb.Focus(); // put focus to facilitate entry
            }
            return isValid;
        }
        ...
        public static bool IsNonNegativeDouble(TextBox tb, string word)
        {
            ...
            if (!Double.TryParse(tb.Text, out value)) // not a correct double
            {
                isValid = false;
        ...
        /// <param name="word">word for for the error message</param>
        /// <returns>true if valid, false if not</returns>
        public static bool IsNonNegativeDecimal(TextBox tb, string word)
        {
            bool isValid = true;
            decimal value; // for TryParse
            if (!Decimal.TryParse(tb.Text, out value)) // not a correct decimal
            {
                isValid = false;
                MessageBox.Show(word + " should be a number", "Input Error");
                tb.SelectAll(); // select content of the text box for replacement
